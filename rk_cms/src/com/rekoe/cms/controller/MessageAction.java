...
import com.rekoe.cms.model.Message;
import com.rekoe.cms.model.MessageType;
import com.rekoe.cms.model.PageModel;
import com.rekoe.cms.utils.FilterHTMLTag;
...
public class MessageAction extends BaseAction {
	...
	public View messagemanager(HttpServletRequest req,
			@Param("tid") Integer tid, @Param("currentPage") int currentPage) {
		...
		List<Message> list = basicDao.searchByPage(Message.class,
				condition.desc("insertTime"), currentPage,
				SystemContext.PAGE_SIZE);
		PageModel<Message> pm = new PageModel<Message>(list, maxPage);
		req.setAttribute("pm", pm);
		req.setAttribute("alltype",
				messageTypeDao.search(MessageType.class, "sortNumber"));
		if (tid == null) {
			tid = 0;
		}
	...
	@Ok("json")
	public String update(@Param("::mess.") Message mess) {
		// MessageDao dao=new MessageDao(ioc);
	...
	@At("/message/add")
	@Ok("json")
	public String add(@Param("::mess.") Message mess) {
		// MessageDao dao=new MessageDao(ioc);
