...
public class StartSetup implements Setup {
	...
	public void initFreeMarkerConfigurer(String path, Ioc ioc)
			throws IOException, TemplateException {
		FreeMarkerConfigurer freeMarkerConfigurer = ioc
				.get(FreeMarkerConfigurer.class);
	...
	public void init(NutConfig nConfig) {
		/*
		 * Ioc ioc = nConfig.getIoc(); Dao dao = ioc.get(Dao.class);
		 * 
		 * String path = nConfig.getServletContext().getRealPath("WEB-INF"); Ioc
		 * ioc = nConfig.getIoc(); try { initFreeMarkerConfigurer(path,ioc); }
		 * catch (IOException e) { e.printStackTrace(); } catch
		 * (TemplateException e) { e.printStackTrace(); } //NutMessageLoader msg
		 * = ioc.get(NutMessageLoader.class);
		 * 
		 * BasicDao basicDao = ioc.get(BasicDao.class); Dao dao =
		 * ioc.get(Dao.class); Role role = basicDao.findByCondition(Role.class,
		 * "id", "48", "permission");//dao.fetch(Role.class, Cnd.where("id",
		 * "=", 48)); dao.fetchLinks(role, "permission");
		 * System.out.println(role.getPermission()); // List<Role> roleList =
		 * Daos.queryWithLinks(dao, Role.class, null, null, "permission"); //
		 * for(Role role:roleList) // { // List<Permission>list =
		 * role.getPermission(); // for(Permission permission:list) // { //
